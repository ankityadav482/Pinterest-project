...
function searchfunctionality(){
    ...
    var input = document.querySelector("#second input")
    ...
    .addEventListener("input",function(){
        const filteredArray = arr.filter(item => item.name.toLowerCase().startsWith(input.value)); 
        var clutter="";
        filteredArray.forEach(function(obj){
            clutter+=`  <h5><i class="ri-search-line"></i>  ${obj.name}</h5>`
        })
